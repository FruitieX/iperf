...
iperf_stats_callback(struct iperf_test *test)
{
	...
	if (test->protocol->id == Ptcp) {
		...
		if (test->sender && test->sender_has_retransmits) {
		    ...
		    temp.snd_ssthresh = get_snd_ssthresh(&temp);
		    if (temp.snd_cwnd > rp->stream_max_snd_cwnd) {
                rp->stream_max_snd_cwnd = temp.snd_cwnd;
		    }
		    if (temp.snd_ssthresh > rp->stream_max_snd_ssthresh) {
                rp->stream_max_snd_ssthresh = temp.snd_ssthresh;
		    }
...
iperf_print_results(struct iperf_test *test)
{
	...
	if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
		...
		if (test->json_output)
		    cJSON_AddItemToObject(json_summary_stream, "sender", iperf_json_printf("socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  retransmits: %d  max_snd_cwnd:  %d  max_snd_ssthresh:  %d  max_rtt:  %d  min_rtt:  %d  mean_rtt:  %d", (int64_t) sp->socket, (double) start_time, (double) end_time, (double) end_time, (int64_t) bytes_sent, bandwidth * 8, (int64_t) sp->result->stream_retrans, (int64_t) sp->result->stream_max_snd_cwnd, (int64_t) sp->result->stream_max_snd_ssthresh, (int64_t) sp->result->stream_max_rtt, (int64_t) sp->result->stream_min_rtt, (int64_t) ((sp->result->stream_count_rtt == 0) ? 0 : sp->result->stream_sum_rtt / sp->result->stream_count_rtt)));
		else
...
print_interval_results(struct iperf_test *test, struct iperf_stream *sp, cJSON *json_interval_streams)
{
	...
	if (test->sender && test->sender_has_retransmits) {
	    ...
	    if (test->json_output)
		cJSON_AddItemToArray(json_interval_streams, iperf_json_printf("socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  retransmits: %d  snd_cwnd:  %d  snd_ssthresh:  %d  rtt:  %d  omitted: %b", (int64_t) sp->socket, (double) st, (double) et, (double) irp->interval_duration, (int64_t) irp->bytes_transferred, bandwidth * 8, (int64_t) irp->interval_retrans, (int64_t) irp->snd_cwnd, (int64_t) irp->snd_ssthresh, (int64_t) irp->rtt, irp->omitted));
	    else {
